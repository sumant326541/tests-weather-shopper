...
describe('Shop for moisturizers or suncreens based on temprature', () => {
  ...
  it('Should Shop for moisturizers if the weather is below 19 and Shop for suncreens if the weather is above 34 degrees', () => {
    cy.get('#temperature').then(($temp) => {
      ...
      cy.get('.text-justify').then(($PaymentConfirmation) => {
        const PaymentConfirmationText = $PaymentConfirmation.text().trim();
        const successfulPaymentText = 'Your payment was successful. You should receive a follow-up call from our sales team.'
        if (PaymentConfirmationText == successfulPaymentText) {
